...
async function addHand(result, lastSignal) {
    ...
    if (finalPred && finalPred !== '-' && finalPred !== 'T' && actualResult !== 'T') {
...
async function editHistory(handId, newResult) {
    ...
    if (oldData) {
        ...
        await updateExpertPerformance(newData); // (ต้อง "ยัด" (Inject) lastSignal ที่ถูกต้อง... ซับซ้อนมาก)
        // (Placeholder: ตอนนี้การ "แก้ไข" จะ "ทำลาย" สถิติ Winrate)
...
// ========== 6. AI EXPERTS (ผู้เชี่ยวชาญ 8 ตัว) ==========
...
function getFinalSignal(votes, riskThreshold) {
    ...
    const totalExperts = voteKeys.length;
    ...
    let condition = `รอเงื่อนไข (ต้องการ ${riskThreshold}/${totalExperts} เสียง)`;
    if (finalPrediction !== 'WAIT') {
        condition = `Hunter Signal (${maxVotes}/${totalExperts} เสียง)`;
    } else if (maxVotes >= riskThreshold) {
...
async function retrainFromCSV(csvString) {
    ...
    for (let i = 0; i < totalLines; i += chunkSize) {
        ...
        for (const line of chunk) {
            ...
            // (‼️‼️ นี่คือ "การแก้ไข" บั๊ก Winrate ‼️‼️)
            // (เรา "ไม่" รู้ Signal ในอดีต... ดังนั้น "บังคับ" ให้เป็น null)
            hand.signalWin = null;
            hand.lastSignal = {};
        ...
        const process = async (command) => {
            switch (command) {
                ...
                case 'DELETE_CURRENT_SHOE':
                    await deleteCurrentShoe(); // (ยังไม่ได้เขียนฟังก์ชันนี้)
...
async function deleteCurrentShoe() {
    // (Placeholder)
    console.log("WORKER: 'DELETE_CURRENT_SHOE' (ยังไม่ได้ทำ)");
    return;
}
async function getShoeSummary() {
    // (Placeholder)
     console.log("WORKER: 'GET_SHOE_SUMMARY' (ยังไม่ได้ทำ)");
    return [{id: 1, isCurrent: true, P: 0, B: 0, T: 0, totalHands: 0}];
}
